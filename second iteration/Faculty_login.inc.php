...
if(!empty($un)&&!empty($pass))
{
	...
	$result = mysqli_query($link,$query) or die("oh nno");
	if($result)
	{
		
		$row_cnt = mysqli_num_rows($result);
		//echo $row_cnt;
		$voot = mysqli_fetch_assoc($result);
		echo $voot['username'];
		$ui= $voot['id'];
		$depa = $voot['dep'];
		...
		{
			?>
			<script> window.alert("login failure")</script>
            <?php
		}
		else
		{
		 ...
		 $user_id = mysqli_data_seek($result,0);
		  $_SESSION['firstname']=$voot['firstname'];
		 $_SESSION['lastname']=$voot['lastname'];
		 $_SESSION['email']=$voot['email'];
		 $_SESSION['mobile']=$voot['mobilenumber'];
         $_SESSION['user_id']=$ui;
